...
const ExamDetailsModal = ({ show, onHide, result }) => {
  ...
  const getGradeColor = (grade) => {
    switch (grade) {
      ...
      case 'B': case 'B+': return 'text-cyan-600 bg-cyan-100';
      case 'C': case 'C+': return 'text-amber-600 bg-amber-100';
  ...
  const getPerformanceLevel = (percentage) => {
    if (percentage >= 90) return { label: 'Outstanding', colorClass: 'text-green-600', Icon: Trophy };
    if (percentage >= 80) return { label: 'Excellent', colorClass: 'text-green-600', Icon: CheckCircle };
    if (percentage >= 70) return { label: 'Very Good', colorClass: 'text-cyan-600', Icon: ArrowUp };
    if (percentage >= 60) return { label: 'Good', colorClass: 'text-cyan-600', Icon: CheckCircle };
    if (percentage >= 50) return { label: 'Average', colorClass: 'text-amber-600', Icon: BarChart3 };
    if (percentage >= 40) return { label: 'Below Average', colorClass: 'text-amber-600', Icon: ArrowDown };
    return { label: 'Needs Improvement', colorClass: 'text-red-600', Icon: AlertTriangle };
  };
  ...
  const downloadPDF = () => {
    ...
    if (strengths.length > 0) {
      ...
      strengths.forEach((strength) => {
        ...
        lines.forEach(line => { checkPageBreak(8); doc.text(line, 20, yPosition); yPosition += 6; });
      });
      yPosition += 5;
    }
    ...
    if (questionsEvaluation?.questions_evaluation?.length > 0) {
      ...
      autoTable(doc, {
        ...
        body: tableData,
        theme: 'grid',
        headStyles: { fillColor: [102, 126, 234], textColor: 255, fontSize: 9, fontStyle: 'bold', halign: 'center' },
    ...
    if (questionsEvaluation?.questions_evaluation?.length > 0) {
      ...
      questionsEvaluation.questions_evaluation.forEach((question, index) => {
        ...
        if (qPercentage === 100) { statusColor = [16, 185, 129]; statusText = 'Correct'; }
        else if (qPercentage === 0) { statusColor = [239, 68, 68]; statusText = 'Incorrect'; }
        else { statusColor = [245, 158, 11]; statusText = 'Partially Correct'; }
        ...
        questionLines.forEach(line => { checkPageBreak(8); doc.text(line, 15, yPosition); yPosition += 5; });
        yPosition += 3;
        ...
        if (question.mistakes_made && question.mistakes_made !== 'None') {
          ...
          mistakesLines.forEach(line => { checkPageBreak(6); doc.text(line, 20, yPosition); yPosition += 5; });
          yPosition += 5;
        }
        ...
        if (question.gap_analysis && question.gap_analysis !== 'No gaps identified') {
          ...
          gapLines.forEach(line => { checkPageBreak(6); doc.text(line, 20, yPosition); yPosition += 5; });
          yPosition += 5;
        }
        yPosition += 5;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      {/* Backdrop */}
      <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={onHide} />

      {/* Modal */}
      <div className={`relative bg-white flex flex-col ${isFullscreen ? "w-full h-full" : "w-full max-w-5xl max-h-[90vh] rounded-2xl mx-4"} shadow-2xl z-10`}>
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-[#0B1120] to-slate-800 text-white rounded-t-none shrink-0" style={isFullscreen ? {} : { borderRadius: '1rem 1rem 0 0' }}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="block px-2 py-0.5 rounded text-xs font-medium bg-slate-200 text-slate-700 w-fit">{result?.exam_type || 'N/A'}</span>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <GraduationCap size={18} className="text-cyan-500" />
                  <div>
                    <span className="text-xs text-slate-500">Class/Section</span>
                    <span className="block font-medium">{result?.class_section || 'N/A'}</span>
                  </div>
                </div>
                {result?.roll_number && (
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="block font-medium">{result.roll_number}</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className={`block font-bold text-lg ${getPercentageTextClass(percentage)}`}>
                      {percentage.toFixed(2)}%
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <PerformanceIcon size={18} className={performance.colorClass} />
                  <div>
                    <span className="text-xs text-slate-500">Performance</span>
                    <span className={`block px-2 py-0.5 rounded text-xs font-semibold w-fit ${
                      percentage >= 70 ? 'bg-green-100 text-green-700' :
                      percentage >= 50 ? 'bg-cyan-100 text-cyan-700' :
                ...
                <table className="w-full border-collapse border border-slate-200 rounded-lg overflow-hidden">
                  <thead>
                    <tr className="bg-slate-100">
                      <th className="border border-slate-200 px-4 py-2 text-center text-sm font-semibold">Question No.</th>
                      <th className="border border-slate-200 px-4 py-2 text-center text-sm font-semibold">Marks Obtained</th>
                      <th className="border border-slate-200 px-4 py-2 text-center text-sm font-semibold">Total Marks</th>
                      <th className="border border-slate-200 px-4 py-2 text-center text-sm font-semibold">Percentage</th>
                    </tr>
                  </thead>
                  <tbody>
                    {questionsEvaluation.questions_evaluation.map((question, index) => {
                      ...
                      return (
                        <tr key={index} className="hover:bg-slate-50">
                          <td className="border border-slate-200 px-4 py-2 text-center font-bold">{question.question_number || `Q${index + 1}`}</td>
                          <td className="border border-slate-200 px-4 py-2 text-center">{marksObtained}</td>
                          <td className="border border-slate-200 px-4 py-2 text-center">{maxMarks}</td>
                          <td className="border border-slate-200 px-4 py-2 text-center">
                            <span className={`inline-block px-2 py-0.5 rounded text-xs font-semibold ${getPercentageBadgeClass(qPercentage)}`}>
            ...
            <div>
              ...
              {questionsEvaluation.questions_evaluation && questionsEvaluation.questions_evaluation.length > 0 ? (
                questionsEvaluation.questions_evaluation.map((question, index) => {
                  ...
                  return (
                    <div key={index} className="mb-4 rounded-xl border border-slate-200 overflow-hidden">
                      {/* Question Header */}
                      <div className={`flex items-center justify-between px-4 py-3 ${
                        qPercentage >= 75 ? 'bg-green-50 border-b border-green-200' :
                        qPercentage >= 50 ? 'bg-amber-50 border-b border-amber-200' :
                        'bg-red-50 border-b border-red-200'
                      }`}>
                        <div className="flex items-center gap-3">
                          <div className="flex items-center gap-2 font-bold">
                            <ClipboardCheck size={16} className="text-[#00A0E3]" />
                        ...
                        <div className="flex items-center gap-4">
                          <div className="text-right">
                            ...
                            <span className="block text-xs text-slate-400">marks</span>
                          </div>
                          <div className={`text-lg font-bold ${getPercentageTextClass(qPercentage)}`}>
                          ...
                          {question.mistakes_made && question.mistakes_made !== 'None' && (
                            <div className="rounded-lg border border-red-200 bg-red-50 p-3">
                              ...
                              {question.mistake_section && question.mistake_section !== 'N/A' && (
                                <div className="mt-2 text-xs text-red-500">Section: {question.mistake_section}</div>
                              )}
                            </div>
                          )}
          ...
          {result?.detailed_analysis && (
            <div className="rounded-xl border border-slate-200 p-5">
              ...
              <p className="text-sm text-slate-600">{result.detailed_analysis}</p>
            </div>
          )}
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-200 shrink-0">
          <button onClick={onHide} className="px-4 py-2 rounded-lg text-sm font-medium bg-slate-200 text-slate-700 hover:bg-slate-300 transition-colors">
            Close
          </button>
